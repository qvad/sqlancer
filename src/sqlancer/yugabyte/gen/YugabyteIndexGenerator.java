...
public final class YugabyteIndexGenerator {
    ...
    public static SQLQueryAdapter generate(YugabyteGlobalState globalState) {
        ...
        if (method == IndexType.HASH) {
            ...
            for (int i = 0; i < Randomly.smallNumber() + 1; i++) {
                ...
                if (Randomly.getBoolean()) {
                    ...
                    sb.append("(");
                    YugabyteExpression expression = YugabyteExpressionGenerator.generateExpression(globalState,
                            randomTable.getColumns());
                    sb.append(YugabyteVisitor.asString(expression));
                    sb.append(")");
                }
        ...
        if (Randomly.getBoolean()) {
            sb.append(" WHERE ");
            YugabyteExpression expr = new YugabyteExpressionGenerator(globalState).setColumns(randomTable.getColumns())
                    .setGlobalState(globalState).generateExpression(YugabyteDataType.BOOLEAN);
    ...
    private static String getNewIndexName(YugabyteTable randomTable) {
        List<YugabyteIndex> indexes = randomTable.getIndexes();
        int indexI = 0;
        while (true) {
