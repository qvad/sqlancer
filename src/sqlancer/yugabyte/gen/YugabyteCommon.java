...
public final class YugabyteCommon {
    ...
    public static boolean appendDataType(YugabyteDataType type, StringBuilder sb, boolean allowSerial,
    ...
    public static void addTableConstraints(boolean excludePrimaryKey, StringBuilder sb, YugabyteTable table,
                                           YugabyteGlobalState globalState, ExpectedErrors errors) {
    ...
    public static void addTableConstraint(StringBuilder sb, YugabyteTable table, YugabyteGlobalState globalState,
            ExpectedErrors errors) {
        addTableConstraint(sb, table, globalState, Randomly.fromOptions(TableConstraints.values()), errors);
    }

    private static void addTableConstraint(StringBuilder sb, YugabyteTable table, YugabyteGlobalState globalState,
            TableConstraints t, ExpectedErrors errors) {
        List<YugabyteColumn> randomNonEmptyColumnSubset = table.getRandomNonEmptyColumnSubset();
        List<YugabyteColumn> otherColumns;
        YugabyteCommon.addCommonExpressionErrors(errors);
        switch (t) {
        case CHECK:
            sb.append("CHECK(");
            sb.append(YugabyteVisitor.getExpressionAsString(globalState, YugabyteDataType.BOOLEAN, table.getColumns()));
            sb.append(")");
    ...
    // complete
    private static void appendExcludeElement(StringBuilder sb, YugabyteGlobalState globalState,
        ...
        if (Randomly.getBoolean()) {
            ...
            // append expression
            sb.append("(");
            sb.append(YugabyteVisitor.asString(YugabyteExpressionGenerator.generateExpression(globalState, columns)));
            sb.append(")");
        }
        if (Randomly.getBoolean()) {
            sb.append(" ");
